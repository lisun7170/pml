...
Cleaned data has  `r ncol(training)` columns:
...
Another side note is that `train` function is comparablly slower than `randomForest` in modeling.

Here are the outcomes from the two models -
...
```{r}
system.time(modFit<-randomForest(classe~.,data=training)) #ntree=200 achieves a similar accuracy!
modFit
...
We now switch positions between training dataset and validating dataset. We use the validaing dataset to fit a new random forest model then use the training dataset for a validation. Here is the result
...
The estimated __out of sample error is oos = `r (oos1+oos2)/2*100`%__ which is an average out of sample error rates oos1 and oos2..

The following plot shows where we missed the prediction in the first cross validataion.
