...
oos<-round(1-sum(diag(a))/sum(a),4)
a
```

The estimated __out of sample error is oos = `r oos*100`%__.
...
Notice that the variable `new_window` in testing dataset doesn't have the same levels as that in the training data. We have to add a new level 'yes' into it before it can be fitted into the training model.
